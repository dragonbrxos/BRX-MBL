...
DEPS="python-pip python-pyqt6 base-devel git alsa-lib cairo ffmpeg gdk-pixbuf2 glib2 glibc graphene gtk4 harfbuzz meson ninja libelf libunwind libbsd vulkan-icd-loader openxr"
...
# 3. NOVO: Compilar e instalar o art-standalone localmente (Runtime Android)
echo -e "${GREEN}Compilando e instalando art-standalone (Runtime Android) localmente...${NC}"
cd "$INSTALL_DIR/native_engine/art_standalone"
rm -rf builddir
meson setup builddir --prefix=/usr
meson compile -C builddir
sudo meson install -C builddir
...
# 4. Compilar e instalar o motor ATL localmente
...
# 5. Configurar caminhos de biblioteca para o runtime Android
...
# 6. Configurar o comando global do BRX-MBL
...
# 7. Criar atalho .desktop para o menu de aplicativos
...
# 8. Associar arquivos .apk para abertura com um clique
